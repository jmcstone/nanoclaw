...
**Group queue** at `src/group-queue.ts` — manages per-group container lifecycle. Honors `MAX_CONCURRENT_CONTAINERS`; queues if at limit. `shutdown()` intentionally detaches active containers rather than killing them — in-flight agents finish on idle timeout. Side effect: after a nanoclaw restart, a lingering Madison carries the OLD config (old CLAUDE.md, old mounts) until her idle timeout; fresh spawns pick up new state.

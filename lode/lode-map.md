...
- `plans/active/` — in-progress trackers
  ...
  - [2026-04-cross-lead-workflows](plans/active/2026-04-cross-lead-workflows/tracker.md) — `_Shared/` Obsidian dropbox + `router.forwardTo` for cross-group file/message handoff; mailroom `forward_to_group` rule action so Inbox can route emails to AVP via user-managed rules; AVP `request_email` tool that proxies send/reply/reply_all/forward through Inbox's existing email infra. **Phase 0 — design captured, not started.**

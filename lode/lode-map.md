...
- `plans/active/` — in-progress trackers
  ...
  - [2026-04-mailroom-repo-extraction](plans/active/2026-04-mailroom-repo-extraction/tracker.md) — split mailroom source out of `~/Projects/ConfigFiles/containers/mailroom/mailroom/` into its own GitLab repo at `~/Projects/mailroom/`. ConfigFiles' compose stack slims to `image: registry.local/mailroom:latest`. Migrates the mailroom plan-lode (active + complete plans about mailroom) into the new repo. NanoClaw's `~/containers/nanoclaw` → `~/Projects/nanoclaw/` move is a separate follow-up plan. Triggered by the structural mismatch — mailroom is a sizable app branching aggressively in a repo intended for "minor config tweaks" — and validated by a 90-day git-history pairing analysis showing ~94% of cross-repo coupling is lode-bookkeeping artifact dissolved by the lode migration. **Not started** — Wave 0 (branch greenup) is the precondition.
- `plans/complete/` — historical reference
  ...
  - [2026-04-mailroom-extraction](plans/complete/2026-04-mailroom-extraction/tracker.md) — **closed 2026-04-25.** Extracted mail ingestion + storage + MCP + backfill out of nanoclaw's host process into a standalone Docker stack. Phases M0–M5 (extraction + cutover) + M6 (bridge hardening — Proton publishes only via Docker network, no host ports) + M7 (lode graduation, mapped to `infrastructure/mailroom-mirror.md` + `architecture/madison-pipeline.md` + `infrastructure/madison-pipeline.md` + `infrastructure/mailroom-rules.md`) all shipped. Phases M8/M9 (fresh-install skill updates) deferred to `TD-MAIL-FRESH-INSTALL-SKILLS`. All subsequent waves (5.5–5.8, read-power) layered on this foundation.

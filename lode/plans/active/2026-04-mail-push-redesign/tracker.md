...
- [x] **8.3** QCM poller **RETIRED** *(Jeff confirmed 2026-04-22: Euclid QCM's source system already pushes alerts directly to his main Telegram, so the email-side `qcm_alerts.jsonl` poller was redundant on top of redundancy. Backed up the row to `~/containers/data/NanoClaw/groups/telegram_main/archive-qcm-poller-2026-04-22.json`, deleted from `scheduled_tasks`. Also stripped `qcm_alert: true` from the QCM rule in `rules.json` since nothing reads the JSONL anymore (urgent surface to Madison + label remain). rules-validate clean post-edit. Changelog appended.)*
...
- [x] **10.1** `mcp__inbox__archive(message_ids: string[])` — loop over ids, group by `(source, account_address)` for Proton connection reuse, call existing `applyActions()` engine with `{auto_archive: true}` per id, record per-id result. Returns `{attempted, succeeded, failed: [{message_id, error}]}`. File: `mailroom/src/mcp/tools/archive.ts`. Mirror the logging shape used by `apply_action` so the MCP log stays audit-ready. *(2026-04-22, ConfigFiles `834aae6`; 270 lines; per-message + batch-summary log lines mirror apply_action.ts; Proton grouping by account_address opens one IMAP session per account regardless of id count; tsc clean, 81/81 tests)*
- [x] **10.2** `mcp__inbox__label(message_ids: string[], labels: string[])` — replace-set semantics. Same skeleton as 10.1 with `{label: labels}` as the Actions. *(2026-04-22, ConfigFiles `29cb2a4`; 264 lines; reuses `buildApplyContext` + `extractEmailFromAccountId` exported from apply_action.ts; per-call lock pattern with `releaseLock()` callback; tsc clean, 81/81 tests)*
- [x] **10.3** `mcp__inbox__add_label(message_ids: string[], label: string)` — union semantics. Actions = `{add_label: label}`. *(2026-04-22, ConfigFiles `e92ad60`; 219 lines; single label string per signature; reuses shared apply_action helpers; tsc clean, 81/81 tests)*
- [x] **10.4** `mcp__inbox__remove_label(message_ids: string[], label: string)` — subtract semantics. Gmail only; Proton remove-label is v1 no-op (surface via `labels_remove_skipped` per id). *(2026-04-22, ConfigFiles `2784c54`; 192 lines; per-id `results[]` carries both `labels_removed` and `labels_remove_skipped`; Proton-skipped ids stay in `results[]` and count as succeeded — informational, not failure. Deviation: skipped per-account Proton grouping since remove is a no-op anyway, each id opens its own session matching apply_action.ts; documented in file comment; tsc clean, 81/81 tests)*
- [x] **10.5** Register the four tools in `mailroom/src/mcp/server.ts`. The agent-runner wildcard `mcp__inbox__*` should already cover the allowlist. *(2026-04-22, ConfigFiles `454909b`; descriptors added to `writeTools` array; per-tool dep-guard switch case throws clear "not available: write deps not configured" when `!deps`; tsc clean, 81/81 tests pre-tests-wave)*
- [x] **10.6** Unit tests mirrored from `apply.test.ts` patterns into `archive.test.ts`, `label.test.ts`. Partial-failure case required (at least one id that doesn't exist in store → goes into `failed[]`, other ids still succeed). *(2026-04-22, ConfigFiles `0ea9e9c`; one test file per tool — archive (8) / label (7) / add_label (8) / remove_label (12, including the Proton-skip contract assertion); 25 new tests; 81 → 106 passing. Mid-run correction: assertions adjusted from raw `Actions` shape to `ResolvedActions` (`labels_to_add`/`labels_to_remove`) because `evaluate()` transforms before `applyActions` is called. Note: `label.ts` has its own local `buildApplyContext` unlike the other three that import from `apply_action.ts` — small inconsistency carried from Wave 1, benign)*
- [x] **10.7** Update Madison's `CLAUDE.md` Writes section: direct her to the four verb tools for bulk work ("archive N messages" → `archive([id1, id2, …])`, not N separate `apply_action` calls). Keep `apply_action` documented as the escape hatch for heterogeneous per-message work and flag-flipping. *(2026-04-22, nanoclaw `f7470f0` (lode bookkeeping only); CLAUDE.md +25 lines on data volume — new "Batch verb tools" block with all four signatures, bulk-work directive, `apply_action` narrowed to escape-hatch, truthfulness link reinforcing per-id `results[]` audit trail, action commands table updated)*
- [x] **10.8** Rebuild mailroom image; restart `mailroom-inbox-mcp-1`. Verify Madison's next archive sweep uses `archive` instead of looping `apply_action`. *(2026-04-22, deploy operation — no commit; `dcc build inbox-mcp` from `~/containers/mailroom` symlinked path → `dcc up inbox-mcp` force-recreated container; healthy in 12s, ~15s effective offline window. MCP introspection confirms 12 tools registered including all four new ones. Functional smoke via `mcp__inbox__validate_rules` returned `ok:true, 30 rules, 6 accounts`. Usage-shift observation rolls into 10.9's 24h audit.)*
...
**Phases 1–8 + Phase 10 implementation complete; Phase 9 + Phase 10.9 audit pending.** End-to-end pipeline live; four batch write tools (archive / label / add_label / remove_label) deployed to `mailroom-inbox-mcp-1` and registered. 303/303 nanoclaw tests + 106/106 mailroom tests (was 81; +25 from Phase 10.6) passing.

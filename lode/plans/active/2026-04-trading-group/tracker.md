...
Apply `/add-telegram-swarm` to provision multiple bot identities. **Six agents in three lanes. Each agent owns a specific artifact type — one writer per file; everyone else reads.** This single-writer discipline keeps lanes clean and prevents swarm merge conflicts.

**Research lane** (operates on strategy results, uses AlgoTrader as a black box):
- [ ] **Researcher** — proposes strategies from S&C + web sources, finds variants, summarizes literature. **Maps ingested strategies to existing components** in the library; proposes new component files for genuinely novel pieces (a new exit rule, a new stop pattern). Actively counters the entry-signal bias — an S&C article with a tired entry but a novel stop is still a library-contributing win. *Owns:* `AlgoTrader/Web Research/`, `AlgoTrader/Sources/`, plus component proposals (Documenter merges into the library).
- [ ] **Regime Analyst** — regime classification, performance attribution, weak-regime mapping, regime-targeted candidate selection from the Findings KB. *Owns:* `AlgoTrader/Regime Reports/`.
...
- [ ] **Chat-noise control:** agents post at milestone events only (candidate selected, verdict issued, KB updated) — not running commentary. Documenter posts a single **morning digest** consolidating the night's activity (promotions, rejections, KB deltas, platform changes).
...
| Chat-noise control via milestone-only posts + morning digest | 6 agents chattering per event would drown the channel. Agents post at decision points; Documenter summarizes nightly. |

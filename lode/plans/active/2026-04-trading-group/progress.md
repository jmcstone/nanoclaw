...
- **Researcher role split into Corpus Researcher + Web Researcher — 7-agent roster.** Supersedes earlier "6 agents" decision.
  - Corpus Researcher: internal-first; continuously re-decomposes older ingests, finds composition gaps, spots cross-source patterns, runs re-triage sweeps; responds to quests with internal findings. Owns `Sources/magazine/`, `Sources/book/`, backlog triage.
  - Web Researcher (Research Scout): external-first; continuously scrapes curated sources + opportunistically explores new ones; responds to quests with external findings. Maintains a **scout notebook** (`Web Research/_scout-notebook.md`) as long-term accumulated expertise — productive sources, credible authors, dead ends. Owns `Web Research/`, `Sources/web/`, `Sources/academic_paper/`.
- **Both researchers run continuously and respond to quests in parallel**, not sequentially. Over-search is self-correcting (Corpus dedups at ingestion); under-search is irrecoverable. Agent time is cheap; backtest runs are the scarce resource.
- **Quest is the inter-agent research primitive.** Any agent fires a quest by writing `Web Research/Quests/{YYYY-MM-DD}-{slug}/prompt.md`. Both researchers respond into sibling files (`internal-findings.md` / `external-findings.md`). Requesting agent writes `integration.md` with disposition. Preserves single-writer discipline while enabling parallel research.
- Ownership matrix in findings.md updated; topics-status.md cross-cutting section updated; Phase 7 Research lane expanded from 3 → 4 agents.

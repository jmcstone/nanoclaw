...
export function writeTasksSnapshot(
  ...
  // Mirror to Obsidian for phone/device visibility. Auto-generated — edits
  // get overwritten on the next DB mutation, so Madison is the editor of
  // record. If Obsidian isn't installed, this silently no-ops.
  try {
    if (fs.existsSync(path.dirname(OBSIDIAN_TASKS_DIR))) {
      fs.mkdirSync(OBSIDIAN_TASKS_DIR, { recursive: true });
      const obsidianFile = path.join(OBSIDIAN_TASKS_DIR, `${groupFolder}.json`);
      const snapshot = {
        _note:
          'Auto-generated by NanoClaw. Read-only — edits are overwritten. To change tasks, ask Madison.',
    ...
    logger.warn(
      { groupFolder, err },
      'Failed to mirror tasks snapshot to Obsidian (non-fatal)',
    );
  }
}

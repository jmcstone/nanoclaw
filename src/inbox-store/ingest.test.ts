...
function protonMsg(
  overrides: Partial<ProtonmailIngestInput> = {},
): ProtonmailIngestInput {
  return {
...
it('Protonmail with in_reply_to (no references) uses in_reply_to as thread root', () => {
  ...
  const msg = db
    .prepare('SELECT thread_id FROM messages WHERE message_id = ?')
    .get('protonmail:<msg-002@proton.me>') as { thread_id: string };
  expect(msg.thread_id).toBe('proton:<msg-001@proton.me>');
});
...
it('Protonmail with references uses the first entry as thread root', () => {
  ...
  const msg = db
    .prepare('SELECT thread_id FROM messages WHERE message_id = ?')
    .get('protonmail:<msg-003@proton.me>') as { thread_id: string };
...
it('Gmail and Protonmail messages with same subject never share a thread', () => {
  ingestGmail(
    gmailMsg({ subject: 'Shared subject', thread_id: 'shared-root' }),
  );
  ingestProtonmail(
    protonMsg({ subject: 'Shared subject', source_message_id: 'shared-root' }),
  );
